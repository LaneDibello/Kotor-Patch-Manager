...
HDC MovieDc = nullptr;
HGLRC MovieContext = nullptr;
bool MovieContextSuccessReported = false;
bool MovieContextFailureReported = false;
bool FilterSuccessReported = false;
bool FilterFallbackReported = false;
bool FilterFailureStageReported = false;

bool filteredFrameFailed(const char* stage) {
    if (!FilterFailureStageReported) {
        OutputDebugStringA("[Movie Patch] Mitchell-Netravali failure: ");
        OutputDebugStringA(stage);
        OutputDebugStringA("\n");
        FilterFailureStageReported = true;
    }
    return false;
}
...
void releaseMovieContextInternal() {
    if (MovieContext) {
        if (OpenGl.wglGetCurrentContext() == MovieContext) {
            OpenGl.wglMakeCurrent(nullptr, nullptr);
        }
        OpenGl.wglDeleteContext(MovieContext);
    }
    if (MovieDc && MovieWindow) {
        OpenGl.releaseDC(MovieWindow, MovieDc);
    }
    MovieWindow = nullptr;
    MovieDc = nullptr;
...
bool reportMovieContextFailure() {
    if (!MovieContextFailureReported) {
        OutputDebugStringA(
            "[Movie Patch] Could not create the movie OpenGL context; using original Bink blit.\n");
        MovieContextFailureReported = true;
    }
    return false;
}
...
#undef RESOLVE_SHADER
    if (!ok) {
        ...
        OutputDebugStringA(
            "[Movie Patch] Required GLSL functions are unavailable; using original Bink blit.\n");
    }
    return ok;
}
...
bool ensureMitchellNetravaliProgram(HGLRC context) {
    ...
    if (fragmentShader == 0) {
        OutputDebugStringA(
            "[Movie Patch] Could not create the Mitchell-Netravali shader.\n");
        return false;
    }
    ...
    if (program == 0) {
        OutputDebugStringA(
            "[Movie Patch] Could not create the Mitchell-Netravali program.\n");
        Shader.deleteShader(fragmentShader);
        return false;
    }
    ...
    if (MovieTextureUniform < 0 || SourceSizeUniform < 0 || TextureSizeUniform < 0) {
        OutputDebugStringA(
            "[Movie Patch] Mitchell-Netravali shader uniforms were not found.\n");
        Shader.deleteProgram(program);
        return false;
    }
...
bool renderFilteredFrame(DWORD bink, const MovieFilterAddresses& addresses) {
    if (!resolveOpenGl()) {
        return filteredFrameFailed("required OpenGL 1.1 functions unavailable");
    }

    HGLRC context = OpenGl.wglGetCurrentContext();
    HDC dc = OpenGl.wglGetCurrentDC();
    if (!context || !dc) {
        return filteredFrameFailed("no current OpenGL context or device context");
    }
    resolveActiveTexture(context);
    if (!ensureMitchellNetravaliProgram(context)) {
        return filteredFrameFailed("GLSL 1.20 shader unavailable");
    }
    ...
    if (!ensureTexture(context, width, height)) {
        ...
        return filteredFrameFailed("movie texture allocation failed");
    }
    ...
    if (error != GL_NO_ERROR) {
        return filteredFrameFailed("OpenGL reported an error while drawing");
    }
    if (OpenGl.swapBuffers(dc) == FALSE) {
        return filteredFrameFailed("SwapBuffers failed");
    }
    return true;
}
...
bool prepareMovieContext(void* movie) {
    if (!resolveOpenGl()) {
        return reportMovieContextFailure();
    }
    ...
    if (MovieContext && MovieWindow == window) {
        const bool contextCurrent =
            OpenGl.wglGetCurrentContext() == MovieContext ||
            OpenGl.wglMakeCurrent(MovieDc, MovieContext) != FALSE;
        return contextCurrent || reportMovieContextFailure();
    }
    releaseMovieContextInternal();
    ...
    if (!MovieContextSuccessReported) {
        OutputDebugStringA(
            "[Movie Patch] Movie OpenGL context created.\n");
        MovieContextSuccessReported = true;
    }
    return true;
}
