...
namespace {
    ...
    fs::path GetLogPath() {
        return GetBaseShaderDirectory() / "shader_debug.log";
    }
    ...
    void LogMessage(const std::string& message) {
        ...
        OutputDebugStringA((std::string("[ShaderDebug] ") + message + "\n").c_str());
    }
    ...
    void DumpShaderSource(const std::string& shaderId, const char* data, size_t size) {
        const fs::path dumpPath = GetBaseShaderDirectory() / "dump" / (shaderId + ".arb");
    ...
    void RegisterShader(const std::string& shaderId, GLenum target, GLuint programId, const char* shaderText, size_t shaderSize) {
    ...
    void WINAPI MyGlProgramString(GLenum target, GLenum format, GLsizei len, const void* string) {
        ...
        DumpShaderSource(shaderId, shaderText, shaderSize);
        RegisterShader(shaderId, target, programId, shaderText, shaderSize);
...
extern "C" void __cdecl ShaderDebug_NoOp() {
}
